...
class Venta extends Sistema
{
    ...
    {
        $this->connect();
        $stmt = $this->conn->prepare("SELECT v.id_venta, v.id_cliente, v.fecha FROM venta v;");
        $stmt->execute();
    ...
    function Delete($id_venta)
    {
        $this->connect();
        $stmt = $this->conn->prepare("DELETE FROM venta
        WHERE id_venta=:id_venta;");
        $stmt->bindParam(':id_venta', $id_venta, PDO::PARAM_INT);
        $stmt->execute();
        return $stmt->rowCount();
    ...
    function Update($id_venta, $datos)
    { //datos es un array
        $this->connect();
        $nombre_archivo = $this->upload('empleados');
        if ($nombre_archivo) {
