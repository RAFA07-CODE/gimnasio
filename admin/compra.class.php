...
class Compra extends Sistema
{
    function getAll()
    {
        $this->connect();
        $stmt = $this->conn->prepare("SELECT c.id_compra, c.id_empleado, c.fecha, c.costo FROM compra c;");
        $stmt->execute();
    ...
    function Delete($id_compra)
    {
        $this->connect();
        $stmt = $this->conn->prepare("DELETE FROM compra
        WHERE id_compra=:id_compra;");
        $stmt->bindParam(':id_compra', $id_compra, PDO::PARAM_INT);
        $stmt->execute();
        return $stmt->rowCount();
    ...
    function Update($id_compra, $datos)
    { //datos es un array
        $this->connect();
        $nombre_archivo = $this->upload('empleados');
        if ($nombre_archivo) {
